...
to routing. A single manifest module declares every topic and every event on that
...
the manifest the single source of truth turns both failures into compile-time errors.
...
4. **Payload shape enforcement.** Declared and introspectable in pass 1, enforced in
   pass 2 — now done. See "Payload shapes" below.
...
Spark earns its place on the manifest specifically: nested `topic do message ... end`
...
API. Because payload
shape enforcement is a stated goal, the manifest will grow nested `field` entities
...
Consequently the subscriber consumes the manifest only through `VerifiedPubSub.Info`,
...
lib/verified_pubsub/manifest.ex                 # use Spark.Dsl entry point
...
lib/verified_pubsub/payload.ex                   # runtime payload validation
lib/verified_pubsub/payload_error.ex
lib/verified_pubsub/transformers/validate_fields.ex
lib/verified_pubsub/transformers/define_payload_schemas.ex
...
validate the manifest, `Info` is the only read interface, `Subscriber` handles
...
verbose, but it is the Spark idiom and has room for options — `required: false` now uses
that room.

### Payload shapes

Enforced as of pass 2. A field's type is one of `:string`, `:integer`, `:float`,
`:boolean`, `:atom`, `:map`, `:list`, `:any`, a `{:list, type}` tuple, or a struct
module. `required: false` permits the key to be absent or `nil`.

**The type is validated at compile time.** The Spark schema accepts `:any` for it,
because no Spark type covers both `{:list, type}` tuples and struct modules, so
`VerifiedPubSub.Transformers.ValidateFields` checks it instead. Without that,
`field :id, :strng` compiled in silence and then matched nothing. A struct module is
recognised syntactically by its `Elixir.` prefix rather than by loading it, since the
module may not be compiled yet.

**Validation runs on every broadcast, in every environment**, and reports every problem
at once rather than the first. A dev/test-only mode was rejected: a payload shape that
production data produces and tests never exercise is exactly the case worth catching.
The cost is one pass over the declared fields, negligible beside the send it guards.

**Both `broadcast/4` and `broadcast!/4` raise** `VerifiedPubSub.PayloadError`. A shape
violation is a bug in the calling code, so `{:error, _}` stays reserved for transport
failures a caller might reasonably handle; a caller matching on `:ok` should not be able
to shrug off a malformed message.

**The payload is a map of exactly the declared fields.** This follows from rejecting
undeclared keys: a struct carries `__struct__` plus all of its own keys, so it can never
satisfy a field list. A struct therefore goes *in* a field, and a struct payload gets a
dedicated error showing the declaration to add. A whole-struct payload form was
considered and rejected to keep one model.

**A literal payload map is checked at compile time** with the same rules. Unlike params,
payloads are usually not literal at the call site — they come from a database record — so
this catches a minority of cases, but it is free.

**The schema lives on the manifest** as `__verified_pubsub_fields__/2`, generated by
`VerifiedPubSub.Transformers.DefinePayloadSchemas`, rather than being inlined at each
call site. Inlining would be marginally faster but would go stale in any caller not
recompiled after a manifest change.
...
`broadcast_from/5`, `broadcast_from!/5` — imported by `use VerifiedPubSub, manifest: ...`
and by `use VerifiedPubSub.Subscriber`.

**This replaces an earlier design in this spec**, which generated
`broadcast_campaigns_created!/2` and friends onto the manifest module. That design was
...
2. **The errors are better.** Elixir's suggester does string similarity on function
   names, capped at five and mixing arities. A macro knows the manifest, so it can list
...
3. **Surface area.** 22 generated functions for a two-topic, four-event manifest; ~95
   for five topics and twenty events. Now a fixed seven macros, and the manifest
...
- Every calling module needs `use VerifiedPubSub, manifest: ...`. Smaller in practice
...
- A module binds exactly one manifest; a second `use` with a different manifest raises.
- No autocomplete-driven discovery of the event catalog. The manifest is one file and is
...
`use VerifiedPubSub.Subscriber` imports the manifest's `subscribe_*`/`unsubscribe_*`
...
typo'd topic fails the manifest lookup with its own error.

The compile-time dependency on the manifest survives this: `Verify` reads the manifest at
`@before_compile`, and `mix xref` still records a `(compile)` edge, so editing the
manifest recompiles every subscriber.
...
**Codegen mechanics.** `handle_message` emits no code of its own; it accumulates the
clause AST into a module attribute. `@before_compile` then emits the private dispatch
clauses. Accumulation is required because coverage can only be checked once every clause
has been seen, which is at `@before_compile`.

An earlier revision of this spec justified it differently — that inline emission would
trip Elixir's "clauses with the same name and arity should be grouped together" warning.
That was **wrong**, and was verified so: clauses generated by a macro carry
`generated: true` metadata and Elixir suppresses that warning for them, whether they are
interleaved or not. The accumulation earns its place on the coverage check alone.

The single `handle_info/2` clause is emitted at the `use` site, not at
`@before_compile`, so a user's own `handle_info/2` clauses are matched after it — a
catch-all of theirs then receives non-verified messages without shadowing verified ones.
Call-site line metadata is propagated into the quoted bodies so stack traces point at
user code.

**Contradictory declarations.** Handling and ignoring the same `{topic, event}` is a
compile error. Without the check it compiled, leaving the `ignore_message` clause dead
and emitting only an obscure warning about an unreachable private function.
...
| Invalid field type in the manifest | `Spark.Error.DslError` from `ValidateFields` |
| Payload shape | `CompileError` for a literal map; `VerifiedPubSub.PayloadError` at runtime otherwise |
| Subscriber exhaustiveness and undeclared events | Hand-rolled `@before_compile` diff |

**Manifest checks use Transformers, not Verifiers — verified empirically.** Spark's
...
exists to avoid cross-module compile-time dependencies; our manifest checks reference
...
time, which creates a compile-time dependency on the manifest, so **editing the
manifest recompiles every subscriber**.
...
Generated functions call `Phoenix.PubSub` directly. `use VerifiedPubSub.Manifest` takes
...
- **Manifest errors** are `Spark.Error.DslError` with a `path:` such as
...
1. **DSL parsing** — build manifests, assert `Info` returns the expected topics,
   events, params, and fields.
2. **Manifest transformers** — compile deliberately-broken manifests with
